...
struct TodoListView: View {
    ...
    var body: some View {
        ...
        .alert(
            "To do list \(todoListViewModel.removeTodoCount)개 삭제하시겠습니까?", isPresented: $todoListViewModel.isDisplayRemoveTodoAlert
        ){
            ...
            HStack{
                if !todoListViewModel.isEditTodoMode {
                    Button(
                        ...
                        }, label: { todo.selected ? Image("checkVariant2") : Image("checkDefault")
                        }
                    )
                }
                VStack(alignment: .leading, spacing: 5) {
                    Text(todo.title)
                        .font(.system(size: 16))
                        .foregroundColor(todo.selected ? Color.gray : Color.bk)
    ...
    fileprivate var body: some View {
        VStack {
            ...
            HStack{
                ...
                Button(
                    action: ({
                        pathModel.path.append(.todoView)
                    }),
                    label: ({
                        Image("WriteBtn")
                    })
